...
module ArtisansUi
  module Accordion
    # Accordion with Disabled Items (Variant 8)
    # Accordion where specific items can be disabled
    # Exact RailsBlocks implementation
    #
    # @example
    #   <%= render ArtisansUi::Accordion::DisabledComponent.new(items: [
    #     { question: "Enabled Question", answer: "This can be opened" },
    #     { question: "Disabled Question", answer: "This cannot be opened", disabled: true },
    #     { question: "Another Enabled", answer: "This can be opened too" }
    #   ]) %>
    class DisabledComponent < ApplicationViewComponent
      ...
      def render_trigger(item, index)
        tag.h3(data: { state: "closed" }, class: "flex text-base font-semibold mt-2 mb-0") do
          tag.button(
            type: "button",
            data: { accordion_target: "trigger", action: item[:disabled] ? "" : "click->accordion#toggle", state: "closed" },
            aria: { controls: "accordion-content-#{index}", expanded: "false" },
            id: "accordion-trigger-#{index}",
            disabled: item[:disabled],
            class: "flex flex-1 items-center text-left justify-between py-2 font-medium transition-all #{item[:disabled] ? 'opacity-50 cursor-not-allowed' : 'hover:underline [&[data-state=open]>svg]:rotate-180'} focus:outline-neutral-700 dark:focus:outline-white focus:outline-offset-2 px-2"
          ) do
            safe_join([
              item[:question],
              chevron_icon
