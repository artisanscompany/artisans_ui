...
module ArtisansUi
  module Accordion
    # Accordion with Nesting (Variant 7)
    # Accordion that supports nested accordion items
    # Exact RailsBlocks implementation
    #
    # @example
    #   <%= render ArtisansUi::Accordion::NestingComponent.new(items: [
    #     {
    #       question: "Parent Question 1",
    #       answer: "Parent Answer 1",
    #       nested_items: [
    #         { question: "Nested Question 1.1", answer: "Nested Answer 1.1" },
    #         { question: "Nested Question 1.2", answer: "Nested Answer 1.2" }
    #       ]
    #     },
    #     { question: "Parent Question 2", answer: "Parent Answer 2" }
    #   ]) %>
    class NestingComponent < ApplicationViewComponent
      ...
      def render_item(item, index, nested: false)
        tag.div(
          data: { accordion_target: "item", state: "closed" },
          class: "border-b pb-2 border-neutral-200 dark:border-neutral-700 #{nested ? 'ml-4' : ''}"
        ) do
          safe_join([
            render_trigger(item, index, nested),
            render_content(item, index, nested)
          ])
        end
      end

      def render_trigger(item, index, nested)
      ...
      def render_content(item, index, nested)
        tag.div(
          ...
          tag.div(class: "px-2 pb-4 pt-0") do
            safe_join([
              tag.div { raw item[:answer] },
              render_nested_items(item)
            ].compact)
          end
        end
      end

      def render_nested_items(item)
        return unless item[:nested_items]&.any?

        tag.div(class: "mt-2", data: { controller: "accordion" }) do
          safe_join(item[:nested_items].map.with_index do |nested_item, nested_index|
            render_item(nested_item, "nested-#{nested_index}", nested: true)
          end)
        end
      end
