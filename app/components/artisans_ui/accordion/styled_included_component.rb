...
module ArtisansUi
  module Accordion
    # Styled Accordion with Included Answer (Variant 6)
    # Accordion with styled background and included answer (no separation)
    # Exact RailsBlocks implementation
    #
    # @example
    #   <%= render ArtisansUi::Accordion::StyledIncludedComponent.new(items: [
    #     { question: "Question 1", answer: "Answer 1" },
    #     { question: "Question 2", answer: "Answer 2" }
    #   ]) %>
    class StyledIncludedComponent < ApplicationViewComponent
      ...
      def render_trigger(item, index)
        tag.h3(data: { state: "closed" }, class: "flex text-base font-semibold mb-0") do
          tag.button(
            type: "button",
      ...
      def render_content(item, index)
        tag.div(
          ...
          tag.div(class: "px-6 pb-4 pt-0") { raw item[:answer] }
        end
      end
