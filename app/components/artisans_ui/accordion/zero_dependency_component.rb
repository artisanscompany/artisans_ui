...
module ArtisansUi
  module Accordion
    # Zero Dependency Accordion (Variant 10)
    # Pure HTML accordion using details/summary elements (no JavaScript required)
    # Multiple items can be open simultaneously
    # Exact RailsBlocks implementation
    #
    # @example
    #   <%= render ArtisansUi::Accordion::ZeroDependencyComponent.new(items: [
    #     { question: "Question 1", answer: "Answer 1" },
    #     { question: "Question 2", answer: "Answer 2" }
    #   ]) %>
    class ZeroDependencyComponent < ApplicationViewComponent
      ...
      def render_item(item, index)
        tag.details(class: "border-b pb-2 border-neutral-200 dark:border-neutral-700 group") do
          safe_join([
            render_summary(item),
            render_content(item)
          ])
