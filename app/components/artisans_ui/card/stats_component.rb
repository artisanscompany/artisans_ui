...
module ArtisansUi
  module Card
    ...
    class StatsComponent < ApplicationViewComponent
      ...
      def call
        tag.div(
          class: "bg-white border border-black/10 overflow-hidden rounded-xl shadow-xs",
          **@html_options
        ) do
          tag.div(class: "px-4 py-5 sm:p-6") do
            content = []

            content << tag.dt(class: "text-sm font-medium text-neutral-500 truncate") { @label }
            content << tag.dd(class: "mt-1 text-3xl font-semibold text-neutral-900") { @value }
                ...
                trend_content = [
                  tag.div(class: "flex items-baseline") do
                    safe_join([
                      ...
                      @trend_label ? tag.span(class: "ml-2 text-sm text-neutral-500") { @trend_label } : nil
                    ].compact)
                  end
                ]
                    ...
                    tag.div(class: "relative pt-1") do
                      safe_join([
                        tag.div(class: "flex mb-2 items-center justify-between") do
                          safe_join([
                            tag.div do
                              tag.span(class: "text-xs font-semibold inline-block text-neutral-600") { @progress_label }
                            end,
                            tag.div(class: "text-right") do
                              tag.span(class: "text-xs font-semibold inline-block text-neutral-600") { "#{@progress_value}%" }
                            end
                          ])
                        end,
                        tag.div(class: "overflow-hidden h-2 mb-4 text-xs flex rounded bg-neutral-200") do
                          tag.div(
