...
module ArtisansUi
  module Clipboard
    # Icon-only clipboard button with tooltip
    # Exact RailsBlocks implementation
    #
    # @example
    #   <%= render ArtisansUi::Clipboard::IconOnlyComponent.new(
    #     text: "Copy this secret text!",
    #     success_message: "Secret copied!"
    #   ) %>
    class IconOnlyComponent < ApplicationViewComponent
      ...
      def render_copy_content
        tag.div(data: { clipboard_target: "copyContent" }) do
          clipboard_icon
        end
      end

      def render_copied_content
        tag.div(data: { clipboard_target: "copiedContent" }, class: "hidden") do
          check_icon
        end
      end
