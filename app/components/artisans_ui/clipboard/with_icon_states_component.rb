...
module ArtisansUi
  module Clipboard
    # Clipboard button with icon states
    # Shows different icons for copy and copied states with visual feedback
    # Exact RailsBlocks implementation
    #
    # @example
    #   <%= render ArtisansUi::Clipboard::WithIconStatesComponent.new(
    #     text: "https://railsblocks.com",
    #     success_message: "URL Copied!"
    #   ) %>
    class WithIconStatesComponent < ApplicationViewComponent
      def initialize(text:, success_message: "Copied!", **html_options)
        @text = text
      ...
      def render_copied_content
        tag.div(data: { clipboard_target: "copiedContent" }, class: "hidden flex items-center gap-1.5 text-green-400 dark:text-green-500") do
          safe_join([
            tag.span("Copied!"),
            check_icon
          ])
