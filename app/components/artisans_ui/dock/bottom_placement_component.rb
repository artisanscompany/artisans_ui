...
module ArtisansUi
  module Dock
    # Dock menu with bottom placement (desktop) and tooltips above icons
    # Exact RailsBlocks implementation with Motion.dev animations
    #
    # @example
    #   <%= render ArtisansUi::Dock::BottomPlacementComponent.new(
    #     desktop_items: [
    ...
    #     ]
    #   ) %>
    class BottomPlacementComponent < ApplicationViewComponent
      ...
      def call
        tag.div(class: "fixed bottom-0 left-0 right-0 z-50 flex items-center justify-center pb-6 pt-0 pointer-events-none") do
          safe_join([
            render_desktop_dock,
            render_mobile_dock
          ])
