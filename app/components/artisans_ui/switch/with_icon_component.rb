module ArtisansUi
  module Switch
    class WithIconComponent < ApplicationViewComponent
      ...
      def toggle_element
        tag.div(class: "absolute top-[3px] left-[3px] w-[18px] h-[18px] bg-white rounded-full shadow-sm transition-all duration-150 ease-in-out peer-checked:translate-x-4 flex items-center justify-center dark:bg-neutral-200 dark:peer-checked:bg-neutral-800") do
          safe_join([
            x_icon,
            checkmark_icon
          ])
      ...
      def checkmark_icon
        tag.svg(
          ...
          viewBox: "0 0 20 20",
          stroke: "currentColor",
          class: "size-4 text-neutral-700 absolute opacity-0 transition-opacity duration-150 ease-in-out group-has-[:checked]:opacity-100 dark:text-neutral-200"
        ) do
