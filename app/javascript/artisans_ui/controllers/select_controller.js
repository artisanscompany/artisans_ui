...
export default class extends Controller {
  static values = {
    ...
    clearButton: { type: Boolean, default: false }, // Show clear button when typing or option selected (only for single select, this is never shown for multiple select)
  ...
  #getRenderConfig() {
    ...
    return {
      option: renderOption,
      item: renderItem,
      option_create: (data, escape) => `<div class="create">${escape(this.createTextValue)} <strong>${escape(data.input)}</strong>&hellip;</div>`,
      ...
      no_results: (data, escape) => `<div class="no-results">${escape(this.noSearchResultsTextValue)} "${escape(data.input)}"</div>`,
    };
  }
  ...
  #createClearButton() {
    ...
    this.clearButton.setAttribute("data-artisans-ui--select-target", "clearButton");
  ...
  #renderImageItem(data, escape) {
    ...
    return `<div class='${itemClass}'>
      <img src='${escape(data[this.imageFieldValue])}' alt='${escape(label)}' class='size-6 rounded-full object-cover' />
  ...
  #renderApiOption(data, escape) {
    ...
    // Image
    if (this.imageFieldValue && data[this.imageFieldValue]) {
      const imageUrl = data[this.imageFieldValue];
      const label = data[this.labelFieldValue] || data.name || "";
      parts.push(`<img src='${escape(imageUrl)}' alt='${escape(label)}' class='size-8 rounded-full object-cover' />`);
    }
    ...
    // Main label
    const label = data[this.labelFieldValue] || data.name || data.text || "";
    labelParts.push(`<span class='font-medium ${textClass}'>${escape(label)}</span>`);

    // Subtitle
    if (this.subtitleFieldValue && data[this.subtitleFieldValue]) {
      labelParts.push(
        `<span class='text-xs text-neutral-500 dark:text-neutral-400'>${escape(
          data[this.subtitleFieldValue]
        )}</span>`
      );
    }

    parts.push(`<div class='flex flex-col gap-0.5 min-w-0 flex-1'>${labelParts.join("")}</div>`);

    // Badge
    if (this.badgeFieldValue && data[this.badgeFieldValue]) {
      parts.push(
        `<span class='inline-flex items-center rounded-full bg-neutral-100 dark:bg-neutral-800 px-2 py-0.5 text-xs font-medium text-neutral-800 dark:text-neutral-200'>${escape(
          data[this.badgeFieldValue]
        )}</span>`
      );
    }
    ...
    if (this.metaFieldsValue) {
      const fields = this.metaFieldsValue.split(",").map((f) => f.trim());
      const metaParts = fields
        .map((field) => {
          if (data[field]) {
            return `<span class='text-xs text-neutral-500 dark:text-neutral-400'>${escape(data[field])}</span>`;
          }
          return "";
        })
        .filter(Boolean);
      ...
      if (metaParts.length > 0) {
        parts.push(`<div class='flex flex-wrap gap-2 w-full'>${metaParts.join("")}</div>`);
      }
    }

    return `<div class='flex items-center gap-2 flex-wrap'>${parts.join("")}</div>`;
  }

  #renderWithTemplate(template, data, escape) {
    // Simple template replacement: {{field}} gets replaced with data[field]
    return template.replace(/\{\{(\w+)\}\}/g, (match, field) => {
      return escape(data[field] || "");
    });
  }

  #renderLoadingMore() {
    return `<div class="loading-more-indicator flex items-center justify-center gap-2 py-2 text-sm text-neutral-500 dark:text-neutral-400">
  ...
  #hasCustomFields() {
    return (
      ...
      this.subtitleFieldValue ||
      this.badgeFieldValue ||
  ...
  #captureScrollState(url) {
    ...
    const scrollTop = this.select.dropdown_content?.scrollTop || 0;
    const activeValue = this.#getActiveValue();

    return { currentPage: parseInt(currentPage), scrollTop, activeValue };
  }

  #updateVirtualScrollState(url, query, json) {
    const hasMore = this.#detectHasMore(json, json.data);
    const nextPageUrl = hasMore ? this.#buildApiUrl(this.urlValue, query, parseInt(new URL(url).searchParams.get("page")) + 1) : null;

    this.select.setNextUrl(query, nextPageUrl);
  }
  ...
  #handlePostLoadFocus(query, scrollState) {
    if (!query && scrollState.activeValue) {
      const activeOption = this.select.getOption(scrollState.activeValue);
      if (activeOption) {
        this.select.setActiveOption(activeOption);
      }
    } else {
  ...
  #cleanupScrollState() {
    // Cleanup method for scroll state (if needed)
  }

  #getActiveValue() {
    const activeOption = this.select.getActiveOption();
    return activeOption?.dataset?.value || null;
  }

  #shouldLoadMore(query, url) {
    // Check if we should load more results
    return !!url;
  }
  ...
  #focusFirstOption(query) {
    ...
    const options = this.select.dropdown_content.querySelectorAll("[data-selectable]");
    if (options.length > 0) {
      this.select.setActiveOption(options[0]);
    }
  }

  #restoreSelectionAfterLoading(lastActiveValue) {
    if (lastActiveValue) {
      const option = this.select.getOption(lastActiveValue);
      if (option) {
  ...
  #clearAllActiveStates() {
    ...
    const options = this.select.dropdown_content.querySelectorAll(".option");
    options.forEach((opt) => opt.classList.remove("active"));
  }

  #isNavigationKey(key) {
    return ["ArrowUp", "ArrowDown", "Home", "End", "Enter", " "].includes(key) || key.length === 1;
  }

  #findMatchingOption(buffer) {
    const options = Object.values(this.select.options);
    return options.find((opt) => {
      const label = opt[this.labelFieldValue] || opt.text || "";
      return label.toLowerCase().startsWith(buffer);
    });
  }

  #scrollToOption(optionEl) {
    if (!optionEl || !this.select?.dropdown_content) return;

    const container = this.select.dropdown_content;
  ...
  #handleInitialValue() {
    // Ensure initial value is properly set
    if (this.element.value && this.select) {
      this.select.setValue(this.element.value, true);
    }
  }
