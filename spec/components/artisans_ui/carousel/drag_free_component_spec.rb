...
RSpec.describe ArtisansUi::Carousel::DragFreeComponent, type: :component do
  let(:slides) do
    [
  ...
  it "includes carousel controller data attributes with drag-free enabled" do
    render_inline(described_class.new(slides: slides))
  ...
  it "accepts custom html_options" do
    render_inline(described_class.new(slides: slides, id: "drag-free-carousel", data: { test: "value" }))

    expect(rendered_content).to include('id="drag-free-carousel"')
