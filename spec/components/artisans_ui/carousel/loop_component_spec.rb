...
RSpec.describe ArtisansUi::Carousel::LoopComponent, type: :component do
  let(:slides) do
    [
  ...
  it "includes carousel controller data attributes with loop enabled" do
    render_inline(described_class.new(slides: slides))
  ...
  it "accepts custom html_options" do
    render_inline(described_class.new(slides: slides, id: "loop-carousel", data: { test: "value" }))

    expect(rendered_content).to include('id="loop-carousel"')
