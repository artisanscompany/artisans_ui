...
RSpec.describe ArtisansUi::Carousel::LoopDragFreeComponent, type: :component do
  let(:slides) do
    [
  ...
  it "includes carousel controller data attributes with loop and drag-free enabled" do
    render_inline(described_class.new(slides: slides))
  ...
  it "accepts custom html_options" do
    render_inline(described_class.new(slides: slides, id: "loop-drag-free-carousel", data: { test: "value" }))

    expect(rendered_content).to include('id="loop-drag-free-carousel"')
