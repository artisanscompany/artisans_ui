...
RSpec.describe ArtisansUi::Select::WithIconsComponent, type: :component do
  let(:options) do
    [
      { label: "Option 1", value: "1", icon: "https://placehold.co/24x24/blue/white" },
      { label: "Option 2", value: "2", icon: "https://placehold.co/24x24/red/white" },
      { label: "Option 3", value: "3", icon: "https://placehold.co/24x24/green/white" }
    ]
  end
  ...
  it "includes icons data attribute" do
    render_inline(described_class.new(options: options))

    expect(rendered_content).to include('data-artisans-ui--select-icons-value="true"')
  end
  ...
  it "includes icon data attributes on options" do
    render_inline(described_class.new(options: options))

    options.each do |opt|
      expect(rendered_content).to include("data-icon=\"#{opt[:icon]}\"")
    end
  end
