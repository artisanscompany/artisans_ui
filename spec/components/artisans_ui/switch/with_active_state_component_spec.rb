...
RSpec.describe ArtisansUi::Switch::WithActiveStateComponent, type: :component do
  it "renders switch with active state" do
  ...
  it "includes active state classes on toggle element" do
    render_inline(described_class.new)

    expect(rendered_content).to include('group-active:w-[22px]')
    expect(rendered_content).to include('peer-checked:group-active:-ml-1')
  end
  ...
  it "includes toggle element base classes" do
    render_inline(described_class.new)
